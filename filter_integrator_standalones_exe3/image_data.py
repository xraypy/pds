...
def correct_image(image, pixel_map=None):
    ...
    if pixel_map is None or pixel_map.startswith("[]") or pixel_map.startswith("None"):
        return image
    else:
        try:
...
############################################################################
def image_plot(img, fig=None, figtitle="", cmap=None, verbose=False, im_max=None, rotangle=0.0, roi=None):
    """
    show image
...
############################################################################
def sum_plot(image, bgrflag=0, cnbgr=5, cwidth=0, cpow=2.0, ctan=False, rnbgr=5, rwidth=0, rpow=2.0, rtan=False, fig=None):
    """
...
############################################################################
def line_sum_integral(image, sumflag="c", nbgr=0, width=0, pow=2.0, tangent=False, compress=1):
    """
    ...
    * Ibgr = the background intensity
    ...
    """
    # get line sum
    (data, data_idx, bgr) = line_sum(image, sumflag=sumflag, nbgr=nbgr, width=width, pow=pow, tangent=tangent, compress=compress)
    ### integrate
    Itot = data.sum()
    Ibgr = bgr.sum()
    # Itot = num.trapz(data)
...
##############################################################################
def image_bgr(image, lineflag="c", nbgr=3, width=100, pow=2.0, tangent=False, nline=1, filter=False, compress=1, plot=False):
    """
    ...
    if lineflag == "r":
        if nline > 1:
            ...
            for j in range(n):
                ...
                bgr_arr[j, :] = background(line, nbgr=nbgr, width=width, pow=pow, tangent=tangent, compress=compress)
        else:
...
class ImageAna:
    ...
    ############################################################################
    def get_vars(self):
        ret = (self.clpimg, self.bgrimg, self.integrated, self.I, self.Ibgr, self.Ierr, self.I_c, self.I_r, self.Ibgr_c, self.Ibgr_r, self.Ierr_c, self.Ierr_r)
        return ret
...
class ImageScan:
    ...
    def __init__(self, image=[], rois=None, rotangle=None, bgr_params=None, archive=None):
        """
        Initialize
    ...
    def _init_image(self):
        ...
        if len(self.rois) != npts:
            self.rois = []
            for j in range(npts):
                self.rois.append([0, 0, self.image[j].shape[1], self.image[j].shape[0]])
        # init rotangle
    ...
    def plot(self, idx=0, fig=None, figtitle="", cmap=None, verbose=False):
        ...
        """
        image_plot(
            self.image[idx], fig=fig, figtitle=figtitle, cmap=cmap, verbose=verbose, im_max=self.im_max[idx], rotangle=self.rotangle[idx], roi=self.rois[idx]
        )

    ################################################################
    def integrate(self, idx=[], roi=None, rotangle=None, bgr_params=None, bad_points=[], plot=False, fig=None):
        """
        integrate images
...
class _ImageList:
    ...
    ################################################################
    def __init__(self, images, file="images.h5", path=None, setname="S000", descr="Scan data images"):
        self.path = path
        self.file = file
