...
def auto_master(spec_dir, scripts_dir):
    spec_sizes = {}
    while True:  # spec_sizes == {}:
        ...
        for item in spec_files:
            ...
            if item not in spec_sizes:
                ...
                subprocess.call([sys.executable, spectopdf_path, full_item, "", "-a"])
            else:
...
class FileLock(object):
    ...
    def acquire(self):
        """
        Acquire the lock, if possible. If the lock is in use, it check again every `wait` seconds.
        It does this until it either gets the lock or exceeds `timeout` number of seconds, in which case it throws an exception.
        """
