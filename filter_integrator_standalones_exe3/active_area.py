...
def active_area(nm, ki=num.array([0.0, 1.0, 0.0]), kr=num.array([0.0, 1.0, 0.0]), beam=[], det=None, sample=1.0, plot=False, fig=None):
    """
...
def calc_surf_transform(nm):
    ...
    * nm is the surface normal vector, defined in the
    ...
    v = num.array([0.0, -1.0, 0.0])
    v_y = v - (num.dot(v, v_z) * v_z) / (cartesian_mag(v_z) ** 2.0)
...
def surface_intercept_bounds(k, v, diameter):
    ...
    if arg > 0.0:
        mag1 = (-b + num.sqrt(arg)) / (2.0 * a)
        mag2 = (-b - num.sqrt(arg)) / (2.0 * a)
