...
def summarize(lines):
    """Runs through a specfile and grabs all the data, sorted by scan."""

    # This list corresponds to the current set of parameters written by spec to lines beginning #G. If that changes,
    # at least this list will need to be modified, if not more code below.
    # JES modified 6/10/2015 to add 'g_new1', 'g_new2', 'g_new3' after upgrade to SPEC version 6.03.03
    g_labs = [
        ...
        "g_aa", "g_bb", "g_cc", "g_al", "g_be", "g_ga", "g_aa_s", "g_bb_s", "g_cc_s", "g_al_s", "g_be_s", "g_ga_s", "g_h0",
        "g_k0", "g_l0", "g_h1", "g_k1", "g_l1", "g_u00", "g_u01", "g_u02", "g_u03", "g_u04", "g_u05", "g_u10", "g_u11",
        "g_u12", "g_u13", "g_u14", "g_u15", "g_lambda0", "g_lambda1", "g_new2", "g_new3","g_54", "g_55", "g_56", "g_57",
        "g_58", "g_59", "g_60", "g_61", "g_62", "g_H", "g_K", "g_L", "g_LAMBDA", "g_ALPHA", "g_BETA", "g_OMEGA", "g_TTH",
        "g_PSI", "g_TAU", "g_QAZ", "g_NAZ", "g_SIGMA_AZ", "g_TAU_AZ", "g_F_ALPHA", "g_F_BETA", "g_F_OMEGA", "g_F_PSI", 
        "g_F_NAZ", "g_F_QAZ", "g_F_DEL", "g_F_ETA", "g_F_CHI", "g_F_PHI", "g_F_NU", "g_F_MU", "g_F_CHI_Z", "g_F_PHI_Z",
    ...
    summary = []
    lineno = 0
    (spec_name, epoch, mnames, cmnd, date, xtime, g_vals, q, p_vals, atten, energy, lab, aborted) = (
    ...
    for i in lines:
        ...
        # Get the G values
            ...
            g_vals = g_vals + i[3:]
        # Get Q
        # NOTE: This value is useless: it shows what Q was at the end of the previous scan, not what it is going to be.
        elif i.startswith("#Q "):
            q = i[3:]
        ...
        # NOTE: This will also match #ATTEN_FACTORS If that starts getting used again, we can either change
        # this to i.startswith('#ATTEN ') or put it after an i.startswith('#ATTEN_F') check.
        ...
        # Also check for comments following the data containing the word 'aborted'
        elif i.startswith("#L "):
            lab = i[3:].split()
            ## count how many lines of 'data' we have and see if the scan was aborted
            xx = lines[lineno:]
            nl_dat = 0
            aborted = False
            for ii in xx:
            ...
            (cmnd, date, xtime, g_vals, q, p_vals, atten, energy, lab, aborted) = (None, None, None, None, None, None, None, None, None, False)
            point_data = []
            (index, ncols, n_sline) = (0, 0, 0)
...
# Main conversion function, takes user input and converts specified .spc file to .mh5
def spec_to_hdf(args):
    ...
    if len(args) == 0:
        ...
        if os.path.isfile(output):
            choice = input("File already exists: (A)ppend, over(W)rite, or (C)ancel (A/W/C)? ").lower()
            if choice == "c":
                return
            elif choice == "":
                choice = "a"
    ...
    # Obtain a lock for the HDF file. If the file is already locked, retry for 10 seconds before returning.
    ...
    for scan in summary:
        ...
        try:
            if len(scan_group["point_data"]) == scan["nl_dat"]:
                print(f"Skipping scan {scan["index"]}")
                continue
        ...
        if scan.get("cmd", "").split()[0] == "a2scan":
            for key in scan.keys():
                if key == "cmd":
                ...
                elif key not in ["labels", "point_data", "g_labs", "mnames", "G", "P"]:
                    scan_key = scan[key]
                    if scan_key is None:
                        scan_key = "None"
                    ...
                    #     try:
                    #         scan_group.attrs['init_epoch'] = time.mktime(time.strptime(scan_key))
                    #     except:
                    #         pass
            ...
            for key in scan.keys():
                if key == "cmd":
                    ...
                    # if key.startswith('date'):
                    #     try:
                    #         scan_group.attrs['epoch'] = time.mktime(time.strptime(scan_key))
                    #     except:
                    #         pass
            ...
            for key in scan.keys():
                if key == "cmd":
                    ...
                    scan_group.attrs["h_val"] = round(float(scan[key].split()[0]), 1)
                    scan_group.attrs["k_val"] = round(float(scan[key].split()[1]), 1)
                elif key not in ["labels", "point_data", "g_labs", "mnames", "G", "P", "Q"]:
                    scan_key = scan[key]
                    if scan_key is None:
                        scan_key = "None"
                    scan_group.attrs[key] = scan_key
        ...
        this_dir = f"{image_dir}S{scan['index']:03d}\\"
    ...
    print("Lock released")
    time2 = time.time()
    print(f"Total time: {time2 - time1 / 60} minutes")
...
class FileLock(object):
    ...
    def acquire(self):
        """
        Acquire the lock, if possible. If the lock is in use, it check again every `wait` seconds. It does this until it either gets
        the lock or exceeds `timeout` number of seconds, in which case it throws an exception.
        """
    ...
    def release(self):
        """
        Get rid of the lock by deleting the lockfile. When working in a `with` statement,
